...
**ชื่อ–นามสกุล:** นาย นนทชัย ไทยตัน  **รหัส:** 68543210012-9 **Sec:** 2 **ชุด:** B
...
| เวลา | งาน | Prompt ที่ถาม | AI ตอบว่าอะไร | ฉันทำอะไรต่อ |
|---|---|---|---|---|
|  14.30 | B1 |  ถ้าจะไล่ debug จากอาการที่โจทย์กำหนด ควรมีวิธีตรวจหาสาเหตุทีละจุดยังไง |  แนะนำให้ดู component ที่เกี่ยวข้อง ตรวจ state, props และการคำนวณข้อมูล แล้วทดสอบหลังแก้แต่ละจุด | ไล่ตรวจโค้ดตามอาการของ bug ทั้ง 6 จุด แล้วแก้และทดสอบเอง |
|  15.00 |  B1 |  ถ้า Summary แสดงจำนวนไม่ตรงกับข้อมูลจริง ควรตรวจส่วนไหนของโค้ดก่อน |  แนะนำให้ตรวจ logic ที่ใช้คำนวณจำนวนแต่ละ status และดูว่ากำลังนับจากข้อมูลชุดไหน |  ตรวจ SummaryPanel และข้อมูลที่ส่งเข้ามา แล้วแก้ logic การนับให้ตรงกับจำนวนการ์ดจริง|
|  15.45 |  B2 |  ถ้าจะเพิ่ม search ใน Dashboard โดยยังมี status filter อยู่ ควรวางแนวคิดการกรองข้อมูลยังไง |  แนะนำให้เก็บค่าค้นหาด้วย state แล้วสร้างเงื่อนไขค้นหาแยกจาก status ก่อนนำมาใช้ร่วมกัน |  เพิ่ม searchText และนำเงื่อนไขค้นหามารวมกับ matchesStatus แล้วทดสอบการค้นหา |
|  16.00 |  B3 |  ถ้าปุ่มรับเรื่องต้องเปลี่ยนสถานะและอัปเดตหน้าโดยไม่ reload ควรเชื่อม component กับ service แบบไหน |  แนะนำให้ส่ง callback จาก Dashboard ไปยังรายการ แล้วเรียก service ก่อนอัปเดต state ด้วยข้อมูลที่ได้กลับมา | ตรวจโค้ด RequestList และ DashboardPage แล้วเชื่อม onAcknowledge กับ updateRequestStatus() จากนั้นทดสอบ REQ-102 |
|  16.20 |  B4 |  ถ้าสร้าง component สำหรับแสดง status ควรจัดการสถานะหลักกับค่าที่ไม่รู้จักยังไง |  แนะนำให้รับ status เป็น prop แยกการแสดงผลของสถานะหลัก และมี fallback สำหรับค่าอื่น |  สร้าง StatusBadge.jsx เอง ทดสอบ 3 สถานะหลัก และทดสอบค่า cancelled เพื่อดูกรณี unknown แล้วลบโค้ดทดสอบออก |
